...
function Todos({ isAuthenticated, setIsAuthenticated }) {
	...
	useEffect(() => {
		const loadData = async () => {
			...
			// Get all todos from the response
			...
			if (filter === 'Overdue') {
				...
				filteredTodos = allTodos.filter(todo => moment(todo.targetDate).isBefore(moment(), 'day'));
			} else if (filter === 'Due Today') {
				// Filter for todos due today
				filteredTodos = allTodos.filter(todo => moment(todo.targetDate).isSame(moment(), 'day'));
			} else {
				// No special filter, use all todos
